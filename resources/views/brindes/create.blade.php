            ...
            <form method="POST" action="salvar" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="custom-select" name="sorteio">
                        ...
                        <option value="{{ $sorteio->sorteio_uid  }}" <?= ((old('sorteio') === $sorteio->sorteio_uid) ? 'selected' : '') ?>>{{ $sorteio->titulo }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <a href="{{route('brindes.index')}}" class="btn btn-secondary float-left showload">
